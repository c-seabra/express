...
const DiscountTemplateItemModalWrapper = ({
  isOpen,
  closeModal,
  prefillData,
  dealId,
  ...
  const [updatePackageItem] = useCommerceUpdateDealItemMutation({
    context,
    onCompleted: () => {
      snackbar('Pricing for discount template detail updated');
    },
    onError: (e) => errorSnackbar(e.message),
    refetchQueries: refetchQueriesContext,
  });
  ...
  return (
    <FormikModal
      alertHeader="Add ticket type constraint to discount template"
      closeModal={closeModal}
      customForm={(props: any) => (
        <DiscountTemplateItemForm
          closeModal={closeModal}
...
export default DiscountTemplateItemModalWrapper;
