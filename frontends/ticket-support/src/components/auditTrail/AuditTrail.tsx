...
const AuditTrail = ({ bookingRef, conferenceSlug, token }: AuditTrailProps) => {
  ...
  const { loading, error, data }: TicketTrailResponse = useQuery(
    TICKET_AUDIT_TRAIL,
    queryOptions,
  );
  ...
  if (assignmentsTrails) {
    for (let index = 0; index < assignmentsTrails.length; index++) {
      ...
      assignmentTrailsVersions = assignmentTrailsVersions.concat(
        element.node.versions,
      );
    }
    ...
    <>
      <StyledContainer>
        ...
        <Table>
          ...
          {!orderedTrails.length && (
            <div>No paper trail records at the moment.</div>
          )}
          {error && <div>{error}</div>}
