...
type UnassignTicketModalProps = Pick<ModalProps, 'isOpen' | 'onRequestClose'> & {
  ticket: Ticket
}
...
const UnassignTicketModal = ({ ticket, isOpen, onRequestClose }: UnassignTicketModalProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <StyledForm>
          <ConfirmationText>
            Are you sure you want to&nbsp;<span>unassign</span>&nbsp;ticket&nbsp;
            <span>{ticket.bookingRef}</span>?
          </ConfirmationText>
          <TextInputField required label="Please enter a reason for this change" name="reason" />
          <CheckboxField label="Send email notification to assignee" name="notify" />
          <WarningMessage>
          ...
          <Modal.DefaultFooter submitText="Confirm" onCancelClick={onRequestClose} />
        </StyledForm>
      </Formik>
    </Modal>
  )
